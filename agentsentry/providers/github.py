...
class GitHubProvider(BaseProvider):
    ...
    @property
    def display_name(self) -> str: return "GitHub"
    @property
    def cloud_provider(self) -> CloudProvider: return CloudProvider.GITHUB
    ...
    def check_permissions(self) -> PermissionStatus:
        ...
        try:
            ...
            return PermissionStatus(
                ok=False, provider_name=self.name, message=str(exc),
            )
    ...
    def scan(self) -> ScanResult:
        ...
        scopes = {s.strip() for s in scopes_header.split(",")} if scopes_header else set()
        ...
        try:
            keys_resp = requests.get(f"{GITHUB_API}/user/keys", headers=headers, timeout=10)
