...
CROWN_JEWEL_KEYWORDS = [
    ...
    "key", "data", "model", "weights", "vault",
]

INTERNET_FACING_KEYWORDS = ["public", "external", "internet", "api", "web", "app", "func"]
...
class AzureScanner:
    ...
    *subscription_client* and the factory callables, or leave them None
    ...
    def scan(self) -> ScanResult:
        ...
        print(f"[AgentSentry/Azure] Done. Found {len(nhis)} NHIs, "
    ...
    def _discover_managed_identities(self, res_client) -> dict[str, dict]:
        ...
        try:
            listed = res_client.resources.list(
                filter=f"resourceType eq 'Microsoft.ManagedIdentity/userAssignedIdentities'"
            )
            for item in listed:
                principal_id = None
                try:
            ...
            print(f"[AgentSentry/Azure] Warning: could not list managed identities: {exc}")
        return identities
    ...
    def _aggregate_role_assignments(self, auth_client, sub_id: str) -> dict[str, dict]:
        ...
        try:
            ...
            print(f"[AgentSentry/Azure] Warning: could not list role assignments: {exc}")
            return principals
        ...
        for assignment in assignments:
            ...
            entry = principals.setdefault(principal_id, {
            ...
            if is_subscription_scope and role_name not in entry["subscription_scope_roles"]:
                entry["subscription_scope_roles"].append(role_name)
            if created_on and (entry["earliest_grant"] is None
                               or created_on < entry["earliest_grant"]):
    ...
    def _principal_findings(
        ...
        if high_priv:
            ...
            findings.append(Finding(
                ...
                risk_level=RiskLevel.CRITICAL if worst == "Owner" else RiskLevel.HIGH,
        ...
        sub_scope = [r for r in info["subscription_scope_roles"] if r in HIGH_PRIV_ROLES]
        if sub_scope:
            findings.append(Finding(
        ...
        for nhi in self._last_result.nhis:
            for role in nhi.attached_policies:
                ...
                matched = (
                    ...
                    if prefix else list(resource_ids)
                )
                if not matched:
