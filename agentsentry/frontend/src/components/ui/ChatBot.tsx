  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (line.startsWith("```")) {
      if (!inCode) { inCode = true; codeLines = []; codeLang = line.slice(3).trim(); }
        ...
        out.push(<pre key={i} style={{ background:"rgba(0,255,136,0.05)",border:"1px solid rgba(0,255,136,0.12)",borderRadius:8,padding:"10px 12px",margin:"8px 0",fontSize:12,fontFamily:"monospace",overflowX:"auto",color:"#00ff88",whiteSpace:"pre-wrap",wordBreak:"break-word" }}>{codeLang&&<div style={{color:"#555",fontSize:10,marginBottom:4}}>{codeLang}</div>}{codeLines.join("\n")}</pre>);
    ...
    out.push(<span key={i} style={{display:"block",minHeight:line===""?"0.6em":undefined}}>{parts.map((p,j)=>{if(p.startsWith("`")&&p.endsWith("`"))return<code key={j} style={{fontFamily:"monospace",fontSize:"0.85em",background:"rgba(0,255,136,0.1)",padding:"1px 5px",borderRadius:4,color:"#00ff88"}}>{p.slice(1,-1)}</code>;if(p.startsWith("**")&&p.endsWith("**"))return<strong key={j} style={{color:"#e0e0e0"}}>{p.slice(2,-2)}</strong>;return<span key={j}>{p}</span>;})}</span>);
  }
...
function TypingDots() {
  return (
    <span style={{display:"inline-flex",gap:3,alignItems:"center",padding:"2px 0"}}>
      {[0,1,2].map(d=>(
        <motion.span key={d} animate={{scale:[0.8,1.2,0.8],opacity:[0.3,1,0.3]}} transition={{duration:1.2,delay:d*0.2,repeat:Infinity,ease:"easeInOut"}}
...
export default function ChatBot() {
  ...
  const panelStyle:React.CSSProperties=isMobile?{position:"fixed",inset:0,zIndex:9000,borderRadius:0,width:"100%",height:"100%",border:"none",display:"flex",flexDirection:"column"}:{position:"fixed",bottom:94,right:24,zIndex:8999,width:"min(390px, calc(100vw - 48px))",height:"min(580px, calc(100vh - 120px))",borderRadius:20,border:"1px solid rgba(0,255,136,0.15)"};
      ...
      <AnimatePresence>
        {open&&(
          <motion.div
            ...
            style={{...panelStyle,background:"#050505",boxShadow:"0 32px 80px rgba(0,0,0,0.75),0 0 0 1px rgba(0,255,136,0.06)",display:"flex",flexDirection:"column",overflow:"hidden"}}
          >
            {/* Header */}
            ...
            <div style={{flex:1,overflowY:"auto",padding:"16px 14px",display:"flex",flexDirection:"column",gap:12,scrollbarWidth:"thin",scrollbarColor:"#222 transparent"}}>
              {msgs.map((m,i)=>(
                <motion.div key={i} initial={{opacity:0,y:8}} animate={{opacity:1,y:0}} transition={{duration:0.18}}
                  ...
                  <div style={{maxWidth:"88%",padding:"10px 14px",borderRadius:m.role==="user"?"16px 16px 4px 16px":"4px 16px 16px 16px",background:m.role==="user"?"linear-gradient(135deg,#00ff88,#00cc6a)":"rgba(255,255,255,0.05)",border:m.role==="assistant"?"1px solid rgba(255,255,255,0.07)":"none",color:m.role==="user"?"#000":"#ccc",fontSize:13,wordBreak:"break-word"}}>
