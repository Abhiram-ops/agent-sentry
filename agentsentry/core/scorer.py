...
IRREVERSIBLE_TOOLS = {
    "send_email", "delete_record", "transfer_funds",
    "deploy", "execute_code", "send_slack_message",
}
...
class NHIScorer:
    ...
    def score(self, nhi: NonHumanIdentity) -> NonHumanIdentity:
        ...
        has_admin_policy = any(
            p in self.ALWAYS_CRITICAL_POLICIES
            for p in nhi.attached_policies
        )
        if has_admin_policy:
    ...
    def _exposure_score(self, nhi: NonHumanIdentity) -> float:
        ...
        if days_since_use is not None and days_since_use > 180:
            score *= 1.5
    ...
    def _ai_amplification_factor(self, nhi: NonHumanIdentity) -> float:
        ...
        if nhi.agent_tools:
            tool_blast = max(
                TOOL_BLAST_SCORES.get(tool, 1.5)
    ...
    def _generate_findings(self, nhi: NonHumanIdentity) -> list[Finding]:
        ...
        if nhi.privilege_score >= 8.0:
            findings.append(Finding(
                ...
                risk_level=RiskLevel.CRITICAL if nhi.privilege_score >= 9.0 else RiskLevel.HIGH,
                ...
                evidence={"privilege_score": nhi.privilege_score,
        ...
        if nhi.is_stale(threshold_days=180):
            findings.append(Finding(
                finding_id="NHI-003",
        ...
        if (nhi.type == NHIType.AI_AGENT
            ...
            findings.append(Finding(
                ...
                evidence={
                    ...
                    "irreversible_tools": [t for t in nhi.agent_tools if t in IRREVERSIBLE_TOOLS],
